...
<div class="card">
    <div class="card-header bg-primary text-white">
        <h5 class="mb-0">✏️ Edit Absensi</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('absensi.update', $absensi) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="peserta_pkl_id" id="peserta_pkl_id" class="form-select @error('peserta_pkl_id') is-invalid @enderror" required>
                    ...
                    @foreach($pesertaPkl as $peserta)
                        <option value="{{ $peserta->id }}" @if(old('peserta_pkl_id', $absensi->peserta_pkl_id) == $peserta->id) selected @endif>
            ...
            <div class="mb-3">
                ...
                <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                    ...
                    <option value="hadir" @if(old('status', $absensi->status) == 'hadir') selected @endif>✓ Hadir</option>
                    <option value="sakit" @if(old('status', $absensi->status) == 'sakit') selected @endif>🏥 Sakit</option>
                    <option value="izin" @if(old('status', $absensi->status) == 'izin') selected @endif>📋 Izin</option>
                    <option value="alpa" @if(old('status', $absensi->status) == 'alpa') selected @endif>✕ Alpa</option>
                </select>
                @error('status')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-3">
                ...
                <textarea name="keterangan" id="keterangan" class="form-control @error('keterangan') is-invalid @enderror" 
                          rows="3">{{ old('keterangan', $absensi->keterangan) }}</textarea>
                @error('keterangan')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
